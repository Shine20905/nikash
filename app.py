...
Nikash - onion quality grading app (SIH 2026, PS 26031).
    ...
    python app.py            start the app (laptop + phone links are printed)
    python app.py --check    grade test.jpg / test.dng in this folder once and exit

Folder:  app.py, nikash/ (code v0.7.1+), models/detector, models/defect, models/calibration
"""
import sys, time, json, base64, pathlib, datetime, socket, uuid
...
# ------------------------------------------------------------------ grading
def grade_images(imgs, lot_id, inspector, models):
    det, cls, cfg, info = models
    lot_id = (lot_id or "").strip() or datetime.datetime.now().strftime("LOT-%Y%m%d-%H%M%S")
    meta = {"lot_id": lot_id, "inspector": (inspector or "-").strip() or "-", "centre": "demo",
    ...
    for g in res.get("gates", []):
        photos.append({"n": g.get("view", 0) + 1, "ok": bool(g.get("ok")), "reason": g.get("reason", ""),
                       "tilt": g.get("tilt_deg"), "height_cm": round(g["camera_height_mm"] / 10) if g.get("camera_height_mm") else None})
    out = {"lot_id": lot_id, "status": res["status"], "photos": photos, "seconds": sec}
    if res["status"] != "GRADED":
        return out

    stem = f"{lot_id}_{datetime.datetime.now():%H%M%S}_{uuid.uuid4().hex[:4]}"
    P.report_pdf(res, ann, str(OUT / f"{stem}.pdf"))
    (OUT / f"{stem}_signed.json").write_text(json.dumps(res, default=float, indent=1))
    ...
    out.update({
        ...
        "pdf": f"/r/{stem}.pdf", "json": f"/r/{stem}_signed.json",
        "onions": [{"id": o["id"], "d": round(o["diameter_mm"], 1), "m": round(o["mass_g"]), "size": o["size_status"],
                    "grade": o["grade"], "defects": o["defects"], "review": o["review"]} for o in res["onions"]],
    })
    print(f"graded {lot_id}: {L['n_onions']} onions, A {L['pct_gradeA_by_weight']:.1f}%, {sec} s")
    return out
...
.btn{display:flex;align-items:center;justify-content:center;gap:8px;width:100%;border:0;border-radius:14px;font-size:17px;font-weight:600;padding:15px;cursor:pointer}
...
.onion{display:flex;align-items:center;gap:10px;padding:10px 0;border-top:1px solid var(--line)}
...
.onion .t{flex:1}.onion .t small{color:var(--mute);display:block;font-size:12px}
.pill{font-size:12px;font-weight:600;padding:3px 9px;border-radius:99px;color:#fff}
...
<main>
 <section id="scan">
  <div class="card stack">
   ...
   <div><label>Inspector</label><input id="insp" type="text" placeholder="optional"></div></div>
  </div>
  <div class="card stack">
   ...
   <div class="meta" id="count">Take 1–3 photos. Live capture only — gallery photos are not accepted.</div>
...
<script>
const $=s=>document.querySelector(s); let photos=[];
if(new URLSearchParams(location.search).get("dev")==="1") document.getElementById("galBtn").classList.remove("hide");
...
async function shrink(file){                       // phone photo -> max 4032 px JPEG before upload
  ...
  try{const url=URL.createObjectURL(file); const img=new Image(); img.src=url; await img.decode();
      const s=Math.min(1,4032/Math.max(img.naturalWidth,img.naturalHeight));
...
async function addFiles(list){
  for(const f of list){ if(photos.length>=3) break; photos.push({file:await shrink(f),name:f.name}); }
  $("#cam").value=""; $("#gal").value=""; draw();
}
function draw(){
  $("#thumbs").innerHTML=""; photos.forEach((p,i)=>{const d=document.createElement("div"); d.className="thumb";
    if(p.file.type.startsWith("image/")&&!/\.dng$/i.test(p.name)){const im=document.createElement("img"); im.src=URL.createObjectURL(p.file); d.appendChild(im);}
  ...
  $("#go").disabled=!photos.length; $("#count").textContent=photos.length?`${photos.length}/3 photo${photos.length>1?"s":""} ready`:"Take 1–3 photos. Live capture only — gallery photos are not accepted.";
}
$("#go").onclick=async()=>{
  ...
  try{const r=await fetch("api/grade",{method:"POST",body:fd}); const j=await r.json(); show(j);}
...
function show(j){
  const R=$("#result"); R.classList.remove("hide");
  if(j.status!=="GRADED"){
  ...
  const on=j.onions.map(o=>`<div class="onion"><div class="badge" style="background:${GC[o.grade]}">${o.id}</div>
     ...
     ${o.review.length?`<div class="flag">⚑ ${esc(nice(o.review))}</div>`:""}</div>
     <span class="pill" style="background:${GC[o.grade]}">${o.grade==="URS"?j.bucket:(o.grade==="FOREIGN"?"Not onion":o.grade)}</span></div>`).join("");
  const ok=j.photos.filter(p=>p.ok).map(p=>`photo ${p.n}: tilt ${Math.round(p.tilt)}°, ${p.height_cm} cm`).join(" · ");
  R.innerHTML=`
  ...
  <div class="card stack"><a class="btn primary" href="${j.pdf}" target="_blank" style="text-decoration:none">⬇ Report (PDF)</a>
    <a class="btn ghost" href="${j.json}" target="_blank" style="text-decoration:none">Signed record (JSON)</a>
...
def serve(models, port=7860):
    ...
    def api_grade(files: List[UploadFile] = File(...), lot_id: str = Form(""), inspector: str = Form("")):
        try:
            ...
            return JSONResponse(grade_images(imgs, lot_id, inspector, models))
    ...
    try:
        ...
        q.make_image().save(ROOT / "phone_qr.png")
        print(f"  Phone link: {phone}")
        print("  QR code saved as phone_qr.png - opening it now; scan it with the iPhone camera")
        import os
        if hasattr(os, "startfile"): os.startfile(str(ROOT / "phone_qr.png"))    # Windows: opens the image
    except ImportError:
...
if __name__ == "__main__":
    ...
    if "--check" in sys.argv:
        ...
        out = grade_images([decode_photo(photo.read_bytes(), photo.name)], "CHECK", "self-test", models)
